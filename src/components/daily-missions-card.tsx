...
import { CheckIcon, ChevronDownIcon, ChevronRightIcon, ChevronUpIcon, DropletIcon, DumbbellIcon, FlameIcon, PlateIcon, TargetIcon } from '@/components/icons';
...
const MISSION_PRESENTATION: Record<DailyMissionKey, { label: string; Icon: ComponentType<{ size?: number; color?: string }> }> = {
  ...
  protein: { label: 'Protein', Icon: TargetIcon },
...
export function DailyMissionsCard({ missions, selectedDate, colors }: { missions: DailyMission[]; selectedDate: string; colors: ThemeColors }) {
  ...
  const accent = equippedTheme?.accent ?? Brand.green;
    ...
    <View style={[styles.card, { backgroundColor: colors.backgroundElement }]}>
      <View style={[styles.header, largeText && styles.largeTextHeader]}>
        ...
        <Pressable accessibilityRole="button" accessibilityLabel={`${balance} Trak Points. Open rewards.`} onPress={() => router.push('/rewards')} style={({ pressed }) => [styles.wallet, { backgroundColor: tint }, largeText && styles.largeTextWallet, pressed && styles.pressed]}>
          ...
          <Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.8} style={[styles.walletValue, { color: colors.text }]}>{balance.toLocaleString()} <Text style={styles.walletUnit}>pts</Text></Text>
        ...
        {missions.map((mission) => {
          ...
          return (
            <Pressable
              ...
              <Text style={[styles.indicatorLabel, { color: colors.text }]}>{label}</Text>
              <Text style={[styles.indicatorProgress, { color: mission.complete ? accent : colors.textSecondary }]}>{compactMissionProgress(mission)}</Text>
            </Pressable>
          );
        })}
      </View>
      ...
      <View style={[styles.footer, { borderTopColor: colors.backgroundSelected }]}>
        ...
        <Pressable accessibilityRole="button" accessibilityState={{ expanded }} onPress={() => setExpanded((current) => !current)} style={({ pressed }) => [styles.disclosure, pressed && styles.pressed]}>
          <Text style={[styles.disclosureText, { color: colors.text }]}>{expanded ? 'Hide missions' : 'View missions'}</Text>
          {expanded ? <ChevronUpIcon size={16} color={colors.textSecondary} /> : <ChevronDownIcon size={16} color={colors.textSecondary} />}
        </Pressable>
      </View>
      ...
      {expanded && <View style={[styles.list, { borderTopColor: colors.backgroundSelected }]}>
        ...
        {missions.map((mission, index) => (
          <Pressable key={mission.key} accessibilityRole={viewingToday && !mission.complete && mission.route ? 'button' : undefined} disabled={!viewingToday || mission.complete || !mission.route} onPress={() => mission.route && router.push(mission.route)} style={({ pressed }) => [styles.row, index > 0 && { borderTopColor: colors.backgroundSelected, borderTopWidth: StyleSheet.hairlineWidth }, pressed && styles.pressed]}>
...
const styles = StyleSheet.create({
  ...
  largeTextIndicator: { flexBasis: '28%' },
