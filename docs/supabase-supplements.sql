-- Trak — Supplements feature database setup
-- Paste this whole file into Supabase → SQL Editor → New query → Run.
-- It creates the tables for the supplement checklist + the daily check-offs
-- and locks them down so each user can only read/write their own rows (RLS).
-- Safe to re-run: everything is create-if-not-exists / drop-then-create.

-- 1) Supplements: the user's checklist of supplements (one row per supplement)
...
-- 2) Supplement checks: one row per supplement per day it was taken.
-- The composite primary key makes "taken today" idempotent — checking the
-- same supplement twice in a day can't create a duplicate row. Deleting a
